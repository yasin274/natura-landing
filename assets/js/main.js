...
(() => {
  ...
  /* ── Корзина ──────────────────────────────────────────── */
  /* Раньше здесь был счётчик-пустышка: он рос по клику, но ничего не хранил.
     С появлением бэкенда корзина стала настоящей (товары из API, состояние
     в localStorage, оформление заказа) и переехала в assets/js/shop.js —
     два обработчика на одном значке корзины дрались бы за него. */

  /* Тост нужен и каталогу, чтобы сообщения выглядели одинаково на всём сайте.
     Отдаём одну функцию наружу вместо копии всего блока в соседнем файле. */
  window.naturaToast = toast;
